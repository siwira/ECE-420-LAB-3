...
void gaussianElimination(double **A, double *x, int size, int num_threads){
    ...
    #pragma omp parallel num_threads(num_threads) shared(A, max_row_index, x, pivot) // shared variables
    {
            ...
            for (int row = cur_idx+1; row < size; ++row){
                if (fabs(A[row][cur_idx]) > fabs(A[max_row_index][cur_idx])){ // changed to fabs()
                    #pragma omp critical // critical section to update max_row_index safely
                        if (fabs(A[row][cur_idx]) > fabs(A[max_row_index][cur_idx])) max_row_index = row; // prevent race
                        // printf("Thread %d found a new max at row %d with value %f\n", omp_get_thread_num(), row, A[row][cur_idx]);
                }
            }
